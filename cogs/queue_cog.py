...
class QueueCommandCog(commands.Cog):
    ...
    async def create_queue(self, ctx):
        ...
        if ctx.message.author.voice.channel.id in server_handler.keys():
            ...
            text_id = ctx.message.channel.id
            ...
            if len(user_queue) == 0:
                ...
                current_queue = PlayerQueue(guild_id, voice_id, text_id, user_queue)
                server_handler[text_id] = current_queue
    ...
    async def next_up(self, ctx):
        ...
        msgs = []
        msgs.append(ctx.message)
        queue = await server_handler[ctx.message.channel.id]
        guild = discord.Client.get_guild(queue.guild_id)

        #gets new leader and mentions them
        name_string = await queue.next_user()
        msgs.append(await guild.get_channel(queue.text_id).send(f'{name_string} is up!'))
    ...
    async def add(self, ctx):
        ...
        if len(mentions) == 0:
            ...
            msgs.append(await guild.get_channel(queue.text_id).send('Nobody added! Make sure to use the @ symbol when adding or removing'))
        elif len(mentions) == 1 and mentions[0].bot:
            #if the mention is a bot throw and error
            msgs.append(await guild.get_channel(queue.text_id).send(f'Cannot add {mentions[0].display_name} because they are a bot'))
        else:
            #add user to queue
            for i, user in enumerate(mentions):
                if user.bot:
                    msgs.append(await guild.get_channel(queue.text_id).send(f'Cannot add {user.display_name} because they are a bot'))
                else:
                    await queue.add_user(user.id)
                    added_string = added_string + f'{user.display_name}'

                    if i+1 < len(mentions):
                        added_string = added_string + ', '

            msgs.append(await guild.get_channel(queue.text_id).send(f'{added_string}'))
            await self.update_embed(queue)
    ...
    async def remove(self, ctx, person):
        ...
        msgs = []
        msgs.append(ctx.message)
        queue = await self.get_user_list(ctx)
        guild = discord.Client.get_guild(queue.guild_id)
        ...
        if len(mentions) == 0:
            ...
            msgs.append(await guild.get_channel(queue.text_id).send('Nobody removed! Make sure to use the @ symbol when adding or removing'))
        else:
            #remove listed mentions
            for user in mentions:
                if user in await queue.current_queue():
                    await queue.remove_user(user)
                    removed_list += user.display_name

            msgs.append(await guild.get_channel(queue.text_id).send(f'Removed {removed_list}'))
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        msgs = []
        queue = server_handler.get(user.voice.channel.id)

        if emoji == reactions[0]:
            #gets new leader and mentions them
            name_string = await queue.next_user()
            msgs.append(await queue.text_channel.send(f'{name_string} is up!'))
            await self.update_embed(queue)
            queue.last_event = datetime.datetime.now()
            
        elif emoji == reactions[1]:
            #adds user to queue
            await queue.append_user(user)
            added_string = f'Added {user.display_name}'
            msgs.append(await queue.text_channel.send(f'{added_string}'))
            await self.update_embed(queue)
            queue.last_event = datetime.datetime.now()
            ...
            msgs.append(await queue.text_channel.send(f'{removed_string}'))
            await self.update_embed(queue)
            queue.last_event = datetime.datetime.now()
            ...
            #shuffles queue
            await queue.shuffle_queue()
            msgs.append(await queue.text_channel.send(f'Queue Shuffled!'))
            await self.update_embed(queue)
            queue.last_event = datetime.datetime.now()
